...
const Navbar = () => {
  ...
  return (
    <div className="w-full flex flex-col justify-center items-center backdrop-blur-md shadow-sm shadow-neutral-300 dark:shadow-neutral-800 bg-[var(--navbar)] text-[var(--navbar-foreground)] border-b border-[var(--navbar-border)]">
        ...
        {categories.length > 0 ? (
          <div className="hidden lg:flex w-full gap-x-10 justify-start items-center my-2 mr-5">
            ...
            <Link
              ...
              className="flex justify-start items-center hover:underline hover:text-pink-600 transition-colors text-sm"
            >
              Source
              <IoLogoGithub />
            </Link>
      ...
      <div className="w-full container hidden lg:flex justify-center items-center flex-wrap gap-x-2 gap-y-2 py-2">
        {" "}
        {categories.map((cat) => (
          <Link
            key={cat.id}
            to={`/category/${cat.id}`}
            className="border rounded-md border-transparent py-1 px-3 hover:bg-white dark:hover:bg-neutral-600 transition-colors text-gray-500 dark:text-gray-300 font-semibold"
          >
            {cat.name}
          </Link>
        ))}
