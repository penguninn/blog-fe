...
const CardPost = ({
  ...
  const createdAtLabel = createdDate
    ? (() => {
  ...
  return (
    <div className="blog-article-card col-span-1">
      <Link
        aria-label={`Cover photo of the article titled ${title}`}
      ...
      >
        {coverImageUrl ? (
          <img
            ...
            loading="lazy"
            decoding="async"
            className="block w-full duration-300 hover:scale-[1.05]"
            style={{
              color: "transparent",
            }}
            src={coverImageUrl}
      ...
      <h1 className="blog-article-card-title font-heading text-foreground mx-4 mb-3 block font-extrabold transition-all duration-300 hover:opacity-75 text-xl">
        ...
        <div className="blog-article-card-author-strip mx-4 flex flex-row flex-wrap items-center">
          ...
          <div className="flex flex-col items-start leading-snug">
            {authorName ? (
              authorUrl ? (
                <a
                  className="blog-article-card-author-name mb-1 block font-semibold text-slate-700 dark:text-slate-400"
                  href={authorUrl}
                  target="_blank"
                  rel="noreferrer"
                >
            ...
            {(hasViews || createdAtLabel) ? (
              <div className="blog-article-card-article-meta flex flex-row text-sm">
                <p className="text-slate-500 dark:text-slate-400 flex flex-row items-center">
                  ...
                  {createdAtLabel ? (
                    <time dateTime={createdDate}>{createdAtLabel}</time>
                  ) : null}
                </p>
              </div>
            ) : null}
