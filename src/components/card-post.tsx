            ...
            <div className="flex gap-2">
              {tags?.map((tag) => (
                <Link
                  key={tag.id}
                  to={`/tag/${tag.id}`}
                  className="text-sm min-w-16 text-center text-gray-500 hover:text-blue-500 border border-gray-300 dark:border-gray-700 rounded-md px-2 bg-gray-200 dark:bg-neutral-800"
                >
                  {tag.name}
                </Link>
              ))}
